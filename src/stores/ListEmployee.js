...
import { addDays, startOfWeek, startOfMonth, endOfWeek, endOfMonth, format, formatDistanceToNow, compareAsc } from 'date-fns';
...
export const resultEmployeeStore = defineStore({
    ...
    actions: {
        async getResultEmployee(id) {
            try {
                const reponse = await axios.get(`https://x.ghtk.vn/api/v2/staff/get-work-result?start_date=${this.day}?end_date=${this.day2}&shop_user_id=${id}`, {
                    headers: {
                        'authorization': `Bearer ${authToken.ACCESS_TOKEN}`
                        // 'Content-Type': 'application/x-www-form-urlencoded',
                      },
        ...
        clickXacNhanTuyChon(a, id) {
            if(compareAsc(new Date(this.selectDay), new Date(this.selectDay2)) == 1) {
                this.nhapNgaySai = true;
                setTimeout(() => {this.nhapNgaySai = false}, 3000);
                return;
            }
            ...
            this.getResultEmployee(id);
            if(this.isSelect == 1) { this.isGreenSelect = a };
            if(this.isSelect == 2) { this.isGreen = a };
            this.hienThiTuyChon = !this.hienThiTuyChon;
        },
        clickHuyBoTuyChon() {
            this.hienThiTuyChon = !this.hienThiTuyChon;
...
const api_url_history = 'https://x.ghtk.vn/api/v2/staff/get-history-action?page=1&limit=30&shop_user_id=';
...
export const historyEmployeeStore = defineStore({
    ...
    actions: {
        ...
        countDate(day) {
            return formatDistanceToNow(new Date(day), { locale: viLocale, addSuffix: true });
        },
        showDate(day) {
        ...
        clickToDay(a) {
            ...
            this.activity_history.map((state) => {
                ...
                if(kq == 0) { this.arrayToSearch.push(state) }
            });
            ...
            this.isGreen = a;
            this.isHistory = a;
            (this.arrayToSearch.length == 0) ? this.isHCheck = false :  this.isHCheck = 1
        },
        clickToWeek(a) { 
            this.arrayToSearch = [];
            this.activity_history.map((state) => {
                ...
                if((kq == 1 && kq2 == -1) || (kq == 1 && kq2 == 0) || (kq == 0 && kq2 == -1) || (kq == 0 && kq2 == 0)) { this.arrayToSearch.push(state) }
            });
...
const api_url_address = 'https://x.ghtk.vn/api/fulfilment/v1/shops/get-pick-addresses';
